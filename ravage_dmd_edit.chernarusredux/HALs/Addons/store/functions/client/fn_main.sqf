...
switch (_mode) do {
	...
	case ("onInit"): {
		...
		//--- Setup check-boxes
		{
			...
			[UICTRL(IDC_RscDisplayStore_CHECKBOX4), "STR_HALS_STORE_CHECKBOX_SELLFILTER"],		// sell filter
	...
	case ("LISTBOX"): {
		...
		switch (toUpper _mode) do {
			...
			case ("UPDATE"): {
				...
				_ctrlCheckbox4 = UICTRL(IDC_RscDisplayStore_CHECKBOX4);			// will use for sell filter
				...
				_sellableItems = [];
				if (cbChecked _ctrlCheckbox4) then {
					{
						_sellableItems append _x;
					} forEach [([player] call dmd_fnc_getPlayerInv)];
				};
					...
					if !(cbChecked _ctrlCheckbox1 && {_price > _money}) then {
						if !(cbChecked _ctrlCheckbox2 && {_stock isEqualTo 0}) then {
							if !(cbChecked _ctrlCheckbox3 && {!(_x in _filterItems)}) then {
								if !(cbChecked _ctrlCheckbox4 && {!(_x in _sellableItems)}) then {
	...
	case ("BUTTON"): {
		...
		switch (toUpper _mode) do {
			...
			case ("ENABLED"): {
				...
				_sellButton = UICTRL(IDC_RscDisplayStore_BUTTON_SELL);
				...
				// SELL BUTTON HIGHLIGHTING
				//	_hasItem = (_classname in (items player + assignedItems player + magazines player + weapons player));
				//	_isBkpack = ((backpack player) isEqualTo _classname);
				//	_sellButton ctrlEnable (_hasItem || _isBkpack);

				_hasItem = (_className in ([player] call dmd_fnc_getPlayerInv));
				_sellButton ctrlEnable (_hasItem);
			};
			case ("BUY"): {
				_amount = floor ((parseNumber ctrlText UICTRL(IDC_RscDisplayStore_EDIT)) max 0);
				_price = UIVALUE(IDC_RscDisplayStore_LISTBOX);
			...
			case ("SELL"): {
				...
				_classname = UIDATA(IDC_RscDisplayStore_LISTBOX);
				_price = UIVALUE(IDC_RscDisplayStore_LISTBOX);
				if !(_classname isEqualTo "rvg_money") then { _price = floor(_price * _salePriceCoef); };
				[player, _classname, _price, _amount] remoteExecCall ["HALs_store_fnc_sell", 2];
			};
		};
	};
	case ("TEXT"): /* DONE */ {
