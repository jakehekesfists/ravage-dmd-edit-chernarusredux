				...
				) then
				{
					if (isNull (_objet getVariable ["R3F_LOG_remorque", objNull])) then
					{
						...
						else
						{
							...
							if (typeName _objet_ou_classe == "STRING") then {
								// inidb persistence
								private _objid = _objet getVariable ["unique_id", -1];
								dmd_delete_this_object = ["_objet", _objid];
								publicVariableServer "dmd_delete_this_object";
								dmd_delete_this_object = nil;
								sleep 1;
								////////
								//  deleteVehicle _objet; <-- should be handled by the pveh
							};
						};
					}
					else
